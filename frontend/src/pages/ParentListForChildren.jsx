...
const ParentListForChildren = () => {
  ...
  const handleChange = (e) => {
    axios.post('http://localhost:9002/childcheckParent', user).then((res) => {
      checkChildren(res.data.message);
    }).catch((error) => console.log(error));

    if (hasChildren.length > 0) {
      axios.post('http://localhost:9002/childlistParent', user).then((res) => {
...
export default ParentListForChildren;
